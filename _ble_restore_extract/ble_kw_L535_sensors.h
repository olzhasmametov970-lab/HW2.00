...
inline void initSensors()
{
    ...
    for (uint8_t i = 0; i < g_sensorCount; i++) {
        ...
        g_sensors[i].reading.valid = g_sensors[i].def.enabled;
...
{
    for (uint8_t i = 0; i < g_sensorCount; i++) {
        if (g_sensors[i].def.channel != channel) continue;
        g_calStore.load(
            channel,
            g_sensors[i].def.scaleMin,
            g_sensors[i].def.scaleMax,
...
inline void printSensorsDiag()
{
    for (uint8_t i = 0; i < g_sensorCount; i++) {
        ...
        if (!s.def.enabled) continue;

        const float voltage = s.filteredAdc * ADC_VREF / ADC_MAX;
        Serial.printf(
            "CH%u %-8s pin=%u | ADC=%4.0f | %.2fV | %.2fmA | %.2f (%.1f..%.1f)\n",
            s.def.channel,
            s.def.label,
            s.def.pin,
            s.filteredAdc,
