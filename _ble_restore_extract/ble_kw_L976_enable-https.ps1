...
try {
    ...
    Invoke-Command -Session $session -ScriptBlock {
        ...
        if ($text -notmatch "API_DOMAIN=") {
            Add-Content $envPath "`nAPI_DOMAIN=5-165-27-141.sslip.io`nACME_EMAIL=admin@hydrowin.ru`n"
        }
        else {
            $text = $text -replace "API_DOMAIN\s*=\s*\S+", "API_DOMAIN=5-165-27-141.sslip.io"
    ...
    curl.exe -sI --connect-timeout 10 --max-time 30 "https://5-165-27-141.sslip.io/health"
    Write-Host "`n=== Готово: https://5-165-27-141.sslip.io/v1 ===" -ForegroundColor Green
}
finally {
